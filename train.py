...
class TrainPipeline():
    ...
    def policy_evaluate(self, n_games=10):
        ...
        """
        current_mcts_player = agent_AI(self.policy_value_net.policy_value_fn,
                                         c_puct=self.c_puct,
                                         n_playout=self.n_playout)
        pure_mcts_player = agent(c_puct=5,
        ...
        try:
            for i in range(self.game_batch_num):
                ...
                if (i+1) % self.check_freq == 0:
                    ...
                    self.policy_value_net.save_model('./current_policy_my_'+str(self.count)+'.model')
                    if win_ratio > self.best_win_ratio:
                        print("New best policy!!!!!!!!")
                        self.best_win_ratio = win_ratio
                        # update the best_policy
                        self.policy_value_net.save_model('./best_policy_my.model')
                        if (self.best_win_ratio == 1.0 and
                                self.pure_mcts_playout_num < 5000):
                            self.pure_mcts_playout_num += 1000
                            self.best_win_ratio = 0.0
            end_training = time.time()
            print('total_training time on ',self.game_batch_num , 'games is ',(end_training-start_training))
